...
It can also  be seen from the summary table that there exist few data points
...
Residual sugar distribution is highly skewed. There exist very few extremely
high values but so many extremely low values.
...
It is interesting to see that the distribution is bimodal. Therefore, two 
groups of wines exist. One is sweet (around 10) and one is 
non-sweet(around 1.5). Most probably, people either prefer sweet or 
non-sweet wines.
...
It is difficult to understand the graph using this bin sizes. I should narrow
...
there exist considerable amount of data above 0.05. Let's log transfrom the 
data to obtain a better insight.
...
The distribution of data stays the same. Large amount of data is cumulated 
around 0.5 and there exist a spread of data having value grater than 0.10
...
extremely large variables but most of the data has a bell-shaped normal-like distribution. If the top 1 percentile is omitted the below distribution
is obtained.
...
Most density values are between 0.98 and 1. There exist also a few 
extreme values. Lets omit the top 1 percentile.
...
The data distribution seems bell-shaped and there are no extreme values. 
...
are numeric and quality is an inetger type. Most variables have right-skewed distributions with extremely high values.
...
Althogh Density was not a primary interest, it was found to have a negative relationship with quality and significant correlation (-0.307 ). From box-plot diagram, it can be observed that high quality wines are less dense when 
trimmed.
...
From the above graph, it can be observed that most low quality wines are 
...
High quality wines were found to have high level of alcohol, low level of residual.sugar, low density, high citric acid, low cholorides, 
...
Alcohol and quality has a positive relationship (after quality of 5) and the relationship is very close to linear. Although there exist fewer data points,
there exist a negative relationship between quality values of 3 to 5. 
However, when extremes are trimmed as in the first graph, it is easier to 
observe the trend.
...
This analysis is conducted to explore the features in white wines and their relationships and interactions among them. The main purpose was to 
