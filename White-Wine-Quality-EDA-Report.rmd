...
ggplot(aes(x = chlorides, y = quality), data =df) +
  xlim(0,quantile(df$chlorides,0.99)) +
  geom_point(aes(color = alcohol_grouped), stat = 'summary', fun.y = mean, alpha =0.7) 
```
...
```{r echo=FALSE, Multivariate_alcohol_grouped_Va_Boxplot1}
...
ggplot(aes(x = alcohol, y = quality), data = df) + geom_point(alpha = 0.1, position = position_jitter(h=0), color = 'orange') + 
  xlim(quantile(df$alcohol, 0.01), quantile(df$alcohol,0.99)) +
  geom_line(stat = 'summary', fun.y = mean) + 
  geom_smooth() + ggtitle('Alcohol Wine Quality Graph') + labs(x = " alcohol (% by volume)", y = "quality (score between 0 and 10)")
```

```{r echo=FALSE, Bivariate_Alcohol_Plot1}
ggplot(aes(x = quality, y = alcohol), data = df) + geom_boxplot(aes(group = quality)) + ggtitle('Alcohol Wine Quality Box Plot') + labs(x = " alcohol (% by volume)", y = "quality (score between 0 and 10)")
```
...
ggplot(aes(x = volatile.acidity, y = quality), data =df) +
  geom_line(aes(color = alcohol_grouped), stat = 'summary', fun.y = mean, alpha =0.8) + ggtitle('Wine Quality Volatile Acidity by Alcohol Graph') + labs(x = "volatile acidity (acetic acid - g / dm^3)", y = "quality (score between 0 and 10)")
```
